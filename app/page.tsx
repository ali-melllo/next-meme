    ...
    <section className="flex w-full lg:w-5/12 lg:mx-auto flex-col justify-center p-5">
...

      {
        <div className="flex justify-center mt-5 items-center">
          <Button disabled={startListing || loading} onClick={handlePredictions} className="w-full text-lg font-semibold h-12">
            {loading ?
              <Loader className="size-5 animate-spin" /> : startListing ?
                <div className="flex items-center gap-x-5"> <Loader className="size-5 animate-spin" />Listing ... </div> :
                'Start Analyzing & Prediction'}
          </Button>
        </div>}
      ...
      <div className="mt-5 flex w-full flex-col items-center justify-center gap-y-3">
        {loading ?
          ...
          memeCoins.map((token: any , index:number) => (
            <div key={token.name} className="flex overflow-hidden relative w-full items-center justify-between rounded-xl border-2 p-3 shadow-2xl">
